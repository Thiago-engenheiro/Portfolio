...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const projetos = document.querySelectorAll(".ConteudoProjeto");

  function filtrarProjetos() {

    const tipoSelecionado = filtroTipo.value.toLowerCase();
    const tagSelecionada = filtroTags.value.toLowerCase();

    projetos.forEach((projeto) => {

      const tipoProjeto = projeto.dataset.tipo.toLowerCase();
      const tagsProjeto = projeto.dataset.tags.toLowerCase();

      const correspondeTipo = tipoSelecionado === "all" || tipoProjeto === tipoSelecionado;
